...
# Note: Indices may vary depending on whether the template is native Google Slides
# or converted from PPTX. We try both idx=0 and idx=1 for BODY/SUBTITLE.
_PLACEHOLDER_FIELDS: dict[str, dict[str, tuple[str, int]]] = {
    "TITLE": {
        "center_title": ("CENTERED_TITLE", 0),
        "subtitle": ("SUBTITLE", 0),  # Changed from 1 to 0 for PPTX compat
    },
    "TITLE_AND_BODY": {
        "title": ("TITLE", 0),
        "body": ("BODY", 0),  # Changed from 1 to 0 for PPTX compat
    },
    "TITLE_AND_TWO_COLUMNS": {
        "title": ("TITLE", 0),
        "body_left": ("BODY", 0),  # Changed from 1 to 0
        "body_right": ("BODY", 1),  # Changed from 2 to 1
    },
    "SECTION_TITLE_AND_DESCRIPTION": {
        "title": ("TITLE", 0),
        "subtitle": ("SUBTITLE", 0),  # Changed from 1 to 0
        "body": ("BODY", 0),  # Changed from 2 to 0
    },
    "BIG_NUMBER": {
        "title": ("TITLE", 0),
        "body": ("BODY", 0),  # Changed from 1 to 0
    },
}

_GENERATED_SLIDE_COUNT = 7  # Adjusted for current template (was 12)
...
def _add_slide_content_requests(
    ...
    for field_name, (ph_type, ph_index) in field_map.items():
        ...
        # Insert new content (replacing existing text)
        # We use replaceAllText instead of deleteText+insertText to handle empty placeholders
        requests.append(
            {
                "insertText": {
                    "objectId": object_id,
