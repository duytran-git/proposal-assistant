...
# Shared storage instance for state persistence
    ...
    for i, file_info in enumerate(md_files):
        try:
            drive.upload_file(
                ...
                file_name=file_info.get("name", f"transcript_{i}.md"),
                content=transcript_parts[i].encode("utf-8"),
            )
        except Exception as e:
            logger.warning("Failed to upload transcript to Meetings folder: %s", e)
